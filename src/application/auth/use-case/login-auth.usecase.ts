...
export class LoginAuthUseCase {
    ...
    async execute(input: LoginUserRequestDto): Promise<LoginUserResponseDto> {
        ...
        const [accessToken, refreshToken] = await Promise.all([
            this.tokenService.sign(payload, this.configPort.get<string>('AUTH_ACCESS_TOKEN_EXPIRES')),
            this.tokenService.sign(payload, this.configPort.get<string>('AUTH_REFRESH_TOKEN_EXPIRES'))
        ]);

        await this.refreshTokenService.create(user.id, refreshToken, payload.sessionId);
